...
const S2_DELAY_MS = 2500;
const DS_DELAY_MS = 1200;
const MAX_RETRY = 5;
const RATE_LIMIT_PENALTY_MS = 5000;
const RETRY_JITTER_MS = 700;

const PROGRESS_EVERY = 5; // print progress every N processed items

const SAVE_LOG_FILE = true;
const LOG_DIR = "C:\\Users\\Eric Tseng\\AppData\\Roaming\\Zotero\\Zotero\\scripts\\metadata-fixer\\logs";
const LOG_PREFIX = "v8_1_s2_deepseek_dryrun";

const MAILTO = "piweitseng0921@gmail.com";
const UA = `ZoteroMetadataFixer/1.0 (mailto:${MAILTO})`;

// Fill your real key before running
const DEEPSEEK_API_KEY = "sk-395e033b636f4cdd8bed4e36b1519270";
const DEEPSEEK_MODEL = "deepseek-chat";

// Optional S2 key for higher rate limits
...
async function requestJSON(method, url, body, headers = {}) {
  ...
  for (let i = 0; i < MAX_RETRY; i++) {
    try {
      ...
      let waitMs = delay + jitter(RETRY_JITTER_MS);
      if (status === 429) waitMs += RATE_LIMIT_PENALTY_MS;
      await sleep(waitMs);
...
async function fetchAbstractByDOI(doi) {
  ...
  const json = await requestJSON("GET", url, null, headers);
  return {
...
if (!DEEPSEEK_API_KEY || DEEPSEEK_API_KEY === "YOUR_DEEPSEEK_API_KEY") {
  throw new Error("Please set DEEPSEEK_API_KEY before running.");
}
...
for (const item of candidates) {
  ...
  try {
    ...
    if (!abstract || abstract.length < 80) {
      try {
        ...
        if (s2.abstract && s2.abstract.length > 80) {
          ...
          s2Fetched++;
          await sleep(S2_DELAY_MS + jitter(250));
        } else {
          s2Miss++;
        }
      } catch (_) {
    ...
    try {
      ...
      await sleep(DS_DELAY_MS + jitter(200));
    } catch (e) {
      dsFail++;
  ...
  if (processedTotal % PROGRESS_EVERY === 0 || processedTotal === totalPlanned) {
    ...
    Zotero.debug(`[v8.1] progress ${processedTotal}/${totalPlanned} | elapsed=${formatDuration(elapsed)} | eta=${formatDuration(eta)}`);
  }
}

const elapsedMs = Date.now() - startedAt;
const avgMsPerItem = totalPlanned ? Math.round(elapsedMs / totalPlanned) : 0;
const estPer100 = avgMsPerItem ? formatDuration(avgMsPerItem * 100) : "n/a";
...
const summary = [
  ...
  `delays_ms: s2=${S2_DELAY_MS}, deepseek=${DS_DELAY_MS}, penalty_429=${RATE_LIMIT_PENALTY_MS}, retry=${MAX_RETRY}`,
  ...
  `timing: elapsed=${formatDuration(elapsedMs)}, avg_ms_per_item=${avgMsPerItem}, est_per_100_items=${estPer100}`,
  "",
  "details:",
  ...details,
].join("\n");
