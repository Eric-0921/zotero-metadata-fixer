// v3.2_full_batch_pipeline_logfile.js
...
const MIN_SCORE_CROSSREF = 0.88;
const MIN_SCORE_FALLBACK = 0.80;
const SLEEP_MS = 250;
...
for (let b = 0; b < (AUTO_LOOP ? MAX_BATCHES : 1); b++) {
  ...
  for (const item of items) {
    ...
    try {
      ...
      if (res.status !== "ok") {
        if (res.reason === "no_journal") {
          review++;
          reviewNoJournal++;
          if (WRITE) { clearMetaTags(item); item.addTag(TAG_REVIEW); await item.saveTx(); }
          allLogs.push(`review(no_journal) | ${title.slice(0, 68)} | doi:${res.candidate?.doi || "-"}`);
        } else {
          const reasonText = res.reason || "low_score_or_no_hit";
          // Distinguish true no-hit from low-score-ish fallback failure
          if (reasonText === "low_score_or_no_hit") {
            review++;
            reviewLowScore++;
            if (WRITE) { clearMetaTags(item); item.addTag(TAG_REVIEW); await item.saveTx(); }
            allLogs.push(`review(low_score_or_no_hit) | ${title.slice(0, 68)}`);
          } else {
            nohit++;
...
if (SAVE_LOG_FILE) {
  try {
    savedLogFilePath = await saveLogFile(fullLog);
  } catch (e) {
    allLogs.push(`log_file_error | ${String(e?.message || e)}`);
  }
}
