...
function crYear(m) {
  const p = m?.issued?.["date-parts"]?.[0];
  return p && p[0] ? parseInt(p[0], 10) : null;
}
function crTitle(m) { return (m?.title && m.title[0]) || ""; }
function crJournal(m) {
  return ((m?.["container-title"] && m["container-title"][0]) || "").replace(/&amp;/gi, "&").trim();
}
...
if (!items.length) return `No target items. library=${libraryID}, ONLY_MISSING_FIELDS=${ONLY_MISSING_FIELDS}`;
...
for (const item of items) {
  ...
  try {
    ...
    sample.push(`${mode} | ${title.slice(0, 68)} -> DOI:${newDOI || "-"} | Journal:${newJournal || "-"}`);
    await sleep(SLEEP_MS);
  } catch (e) {
    failed++;
    if (WRITE) {
...
return [
  `WRITE=${WRITE}, MAX_ITEMS=${MAX_ITEMS}, ONLY_MISSING_FIELDS=${ONLY_MISSING_FIELDS}`,
  `library=${libraryID}, total_candidates=${candidates.length}, processed=${items.length}`,
  `checked=${checked}, updated=${updated}, unchanged=${unchanged}, nohit=${nohit}, review=${review}, failed=${failed}`,
  `sample:\n${sample.slice(0, 12).join("\n")}`,
].join("\n\n");
